...
namespace lwt {
    ...
    lwtp::Server::SocketSession::Tag CERT_ROLE_MASK_TAG{};

    void CertRoleInterceptor::HandleClientCertificate(lwtp::Server::SocketSession& session, const mbedtls_x509_crt* clientCert)
    {
        if (!clientCert) {
            return;
        }
